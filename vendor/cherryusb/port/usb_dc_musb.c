    ...
    switch (usb_ep0_state) {
        ...
        case USB_EP0_STATE_OUT_DATA:
            if (ep0_status & USB_CSRL0_RXRDY) {
                ...
                /* The data stage also ends when the requested length is
                 * reached: a wLength that is a multiple of the packet size
                 * (every DFU block) never produces the short packet the
                 * original code waited for. */
                if (read_count < g_musb_udc.out_ep[0].ep_mps ||
                    g_musb_udc.out_ep[0].actual_xfer_len >= g_musb_udc.out_ep[0].xfer_len) {
