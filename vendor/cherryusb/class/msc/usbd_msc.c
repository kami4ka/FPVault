...
/* READ CAPACITY(16) / SERVICE ACTION IN - macOS probes this before
 * falling back to READ CAPACITY(10); without it the bulk pipe desyncs
 * and the host abandons the device. 32-byte response, big-endian. */
static bool SCSI_readCapacity16(uint8_t busid, uint8_t **data, uint32_t *len)
{
    uint32_t nbr = g_usbd_msc[busid].scsi_blk_nbr[g_usbd_msc[busid].cbw.bLUN];
    uint32_t bsz = g_usbd_msc[busid].scsi_blk_size[g_usbd_msc[busid].cbw.bLUN];
    ...
    capacity16[7] = (uint8_t)(((nbr - 1) >> 0) & 0xff);
    capacity16[8] = (uint8_t)((bsz >> 24) & 0xff);
    capacity16[9] = (uint8_t)((bsz >> 16) & 0xff);
    capacity16[10] = (uint8_t)((bsz >> 8) & 0xff);
    capacity16[11] = (uint8_t)((bsz >> 0) & 0xff);

    memcpy(*data, capacity16, 32);
    *len = (g_usbd_msc[busid].cbw.dDataLength < 32) ? g_usbd_msc[busid].cbw.dDataLength : 32;
    return true;
}
